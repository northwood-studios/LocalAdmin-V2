    ...
    public class Config
    {
        ...
        public static Config DeserializeConfig(string[] lines)
        {
            ...
            foreach (string? line in lines)
            {
                ...
                string[]? sp = line.Split(SplitArray, StringSplitOptions.None);
                if (sp.Length != 2)
                    continue;

                switch (sp[0].ToLowerInvariant())
                {
                    case "restart_on_crash" when bool.TryParse(sp[1], out bool b):
                        cfg.RestartOnCrash = b;
                        break;

                    case "la_live_view_use_utc" when bool.TryParse(sp[1], out bool b):
                    ...
                    case "la_show_stdout_and_stderr" when bool.TryParse(sp[1], out bool b):
                        cfg.LaShowStdoutStderr = b;
                        break;

                    case "la_no_set_cursor" when bool.TryParse(sp[1], out bool b):
                        cfg.LaNoSetCursor = b;
                        break;

                    case "enable_la_logs" when bool.TryParse(sp[1], out bool b):
                        cfg.EnableLaLogs = b;
                        break;

                    case "la_logs_use_utc" when bool.TryParse(sp[1], out bool b):
                        cfg.LaLogsUseUtc = b;
                        break;

                    case "la_log_auto_flush" when bool.TryParse(sp[1], out bool b):
                        cfg.LaLogAutoFlush = b;
                        break;

                    case "la_log_stdout_and_stderr" when bool.TryParse(sp[1], out bool b):
                        cfg.LaLogStdoutStderr = b;
                        break;

                    case "la_delete_old_logs" when bool.TryParse(sp[1], out bool b):
                        cfg.LaDeleteOldLogs = b;
                        break;

                    case "la_logs_expiration_days" when ushort.TryParse(sp[1], out ushort b):
                        cfg.LaLogsExpirationDays = b;
                        break;

                    case "delete_old_round_logs" when bool.TryParse(sp[1], out bool b):
                        cfg.DeleteOldRoundLogs = b;
                        break;

                    case "round_logs_expiration_days" when ushort.TryParse(sp[1], out ushort b):
                        cfg.RoundLogsExpirationDays = b;
                        break;

                    case "compress_old_round_logs" when bool.TryParse(sp[1], out bool b):
                        cfg.CompressOldRoundLogs = b;
                        break;

                    case "round_logs_compression_threshold_days" when ushort.TryParse(sp[1], out ushort b):
